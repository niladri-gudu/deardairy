...
export function SettingsModal({
  ...
  // 📊 Storage Stats
  const [stats, setStats] = React.useState({ usedMB: 0, limitMB: 50, fileCount: 0, files: [] });
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (croppedBlob) {
        const { uploadUrl, publicUrl } = await getAvatarPresignedUrl(user.id, "image/jpeg");
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-[95vw] sm:max-w-180 h-[85vh] sm:h-135 p-0 gap-0 border-border/40 bg-background/95 backdrop-blur-xl rounded-3xl sm:rounded-4xl overflow-hidden shadow-2xl flex flex-col sm:flex-row">
          <div className="sr-only">
            ...
            <DialogDescription>Manage your sanctuary identity and archives.</DialogDescription>
          </div>
          ...
          <div className="w-full sm:w-52.5 border-b sm:border-b-0 sm:border-r border-border/20 bg-secondary/10 p-3 sm:p-4 flex flex-row sm:flex-col justify-between items-center sm:items-stretch shrink-0">
            <div className="flex flex-row sm:flex-col items-center sm:items-stretch w-full gap-2 sm:gap-1 overflow-x-auto no-scrollbar">
              <div className="hidden sm:block px-3 py-4 mb-2">
                <span className="text-xl font-black tracking-tighter italic">withink.</span>
              </div>
              {[{ id: "profile", label: "Profile", icon: User }, { id: "appearance", label: "Appearance", icon: Palette }, { id: "data", label: "Data & Media", icon: Database }].map((item) => (
                <button
                  key={item.id}
                  ...
                  className={cn(
                    ...
                    activeTab === item.id ? "bg-foreground text-background shadow-md sm:shadow-lg sm:scale-[1.02]" : "hover:bg-secondary/50 text-muted-foreground",
                  )}
                >
            ...
            <Button variant="ghost" size="sm" className="hidden sm:flex justify-start gap-3 rounded-xl text-muted-foreground hover:text-destructive mt-auto" onClick={async () => { await signOut(); window.location.href = "/"; }}>
            ...
            {activeTab === "profile" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 sm:slide-in-from-right-4 duration-300">
                ...
                <div className="flex flex-col items-center sm:items-start gap-4 mb-2">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="relative group transition-transform active:scale-95">
                    <div className="h-24 w-24 rounded-full border-4 border-background shadow-2xl overflow-hidden bg-secondary/30 relative">
                      {localPreview ? (
                        <Image src={localPreview} alt="Preview" fill className="object-cover" />
                      ) : user.image ? (
                        <Image src={user.image} alt={user.name} fill className="object-cover" />
                      ) : (
                        <div className="h-full w-full flex items-center justify-center text-muted-foreground"><User size={40} /></div>
                      )}
                    </div>
                  ...
                  <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">{localPreview ? "Preview ready" : "Click to change avatar"}</p>
                </div>

                <div className="space-y-4">
                  <div className="grid gap-1.5">
                    <label className="text-[10px] uppercase font-bold text-muted-foreground ml-1">Display Name</label>
                    <input value={name} onChange={(e) => setName(e.target.value)} className="bg-secondary/20 p-4 sm:p-3.5 rounded-2xl border border-border/50 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all w-full" />
                  </div>
                  <div className="grid gap-1.5 opacity-60">
                    ...
                    <div className="bg-secondary/10 p-4 sm:p-3.5 rounded-2xl border border-border/50 text-sm font-medium italic">{user?.email}</div>
                  </div>
                </div>

                <Button onClick={handleSaveProfile} disabled={isUpdating || (name === user?.name && !croppedBlob)} className="w-full gap-2 rounded-2xl h-12 font-black uppercase tracking-widest shadow-xl">
                  {isUpdating ? <div className="h-4 w-4 border-2 border-background border-t-transparent animate-spin rounded-full" /> : "Save Changes"} <Save size={16} />
                </Button>

                <div className="pt-4 space-y-4">
                  <div className="grid gap-1.5">
                    <label className="text-[10px] uppercase font-bold text-muted-foreground ml-1">Password</label>
                    <Button variant="outline" className="justify-between rounded-2xl h-12 border-border/40 hover:bg-secondary/20 group transition-all" onClick={handlePasswordReset}>
                      <div className="flex items-center gap-3">
                        <KeyRound size={16} className="text-muted-foreground group-hover:text-primary transition-colors" />
                        <span className="text-xs sm:text-sm font-medium">Reset Password Link</span>
                      </div>
                ...
                <div className="pt-6 border-t border-destructive/20">
                  <h4 className="text-sm font-bold text-destructive mb-1 flex items-center gap-2"><UserMinus size={16} /> Danger Zone</h4>
                  <p className="text-[11px] text-muted-foreground mb-4 leading-relaxed">Permanent account deletion. This will wipe all your archives.</p>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 sm:slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {[
                    ...
                    <button key={t.id} onClick={() => setTheme(t.id)} className={cn("h-20 sm:h-28 rounded-[20px] sm:rounded-3xl cursor-pointer transition-all p-4 sm:p-5 flex flex-col justify-between items-start border-2 relative overflow-hidden group", t.color, theme === t.id ? "border-primary ring-2 ring-primary/10 scale-[1.01]" : "border-transparent hover:border-border/60")}>
                      {theme === t.id && (
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {stats.files.length > 0 ? stats.files.map((file: any) => (
                      <div key={file.key} className="relative aspect-square rounded-lg sm:rounded-xl overflow-hidden border border-border/40 bg-muted">
                        <Image src={file.url} fill className="object-cover grayscale hover:grayscale-0 transition-all" alt="R2" />
                      </div>
                    )) : [1, 2, 3, 4].map((i) => (
      ...
      <Dialog open={showCropper} onOpenChange={setShowCropper}>
        <DialogContent className="max-w-[400px] p-0 overflow-hidden bg-background rounded-3xl border-border/40 shadow-2xl">
          <div className="p-6 pb-2 text-center">
            <DialogTitle className="text-lg font-black tracking-tighter uppercase italic">Adjust Avatar</DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">Position your picture.</DialogDescription>
          </div>
          ...
          <div className="p-6 space-y-4">
            <input type="range" min={1} max={3} step={0.1} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="w-full h-1.5 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary" />
            <div className="flex gap-2">
              <Button variant="secondary" className="flex-1 rounded-xl font-bold text-xs" onClick={() => { setShowCropper(false); setImageSrc(null); }}>Cancel</Button>
              <Button className="flex-1 rounded-xl font-bold text-xs" onClick={generatePreview}>Set Preview</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
