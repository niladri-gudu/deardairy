  ...
  const handleSignin = async () => {
    ...
    if (res.error) {
      if (res.error.status === 401) {
        setErrors({ email: "Invalid credentials", password: "Check secret key" });
      } else {
      ...
      <div className="w-full max-w-sm mx-auto space-y-10">
        <div className="space-y-3">
          <h1 className="text-5xl font-black tracking-tighter leading-[0.85]">
            Welcome <br />
            <span className="text-primary/60 italic font-serif font-light text-6xl">back.</span>
          </h1>
        ...
        <div className="space-y-6">
          <div className="space-y-2 relative">
            <div className="flex justify-between items-end">
              <Label className="text-[10px] font-mono uppercase tracking-[0.3em] text-muted-foreground/60 ml-1">Identity.Email</Label>
              {errors.email && <span className="text-[9px] font-mono text-destructive uppercase tracking-tighter animate-in fade-in slide-in-from-right-1">// {errors.email}</span>}
            </div>
            <Input
              type="email"
          ...
          <div className="space-y-2 relative">
            <div className="flex justify-between items-end">
              <Label className="text-[10px] font-mono uppercase tracking-[0.3em] text-muted-foreground/60 ml-1">Secret.Key</Label>
          ...
          <div className="pt-4 space-y-4">
            <Button className="w-full h-14 rounded-full font-bold text-lg hover:scale-[1.02] active:scale-[0.98] transition-all relative overflow-hidden" onClick={handleSignin} disabled={isLoading}>
            ...
            <Button variant="ghost" className="w-full h-12 rounded-full font-mono text-[10px] uppercase tracking-widest text-muted-foreground hover:text-foreground" onClick={() => signIn.social({ provider: "google", callbackURL: "/journal" })}>
        ...
        <div className="flex flex-col items-center gap-4">
          <Link href="/forgot-password" className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground/30 hover:text-primary transition-colors italic">Forgot_Password?</Link>
          <p className="text-center text-[11px] font-medium text-muted-foreground/60">New here? <Link href="/signup" className="text-foreground font-bold border-b border-primary/40 hover:border-primary transition-all pb-0.5 ml-1">Start_New_Journey</Link></p>
        </div>
      </div>
    </div>
  );
